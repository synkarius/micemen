...
public class FilteringBlockIterator<T extends Block> implements Iterator<T> {
    ...
    public FilteringBlockIterator(CheeseGrid grid, Direction dir1, Direction dir2, int x, int y, Class<T> clazz) {
        this.grid = grid;
        this.dir1 = dir1;
        this.dir2 = dir2;
        this.x = x;
     ...
     * optional param
     */
    public FilteringBlockIterator<T> filter(Predicate<T> filter) {
        this.filter = filter;
        return this;
    }
     ...
     * optional param
     */
    public FilteringBlockIterator<T> xLimit(int lx) {
        this.lx = lx;
        return this;
    }
     ...
     * optional param
     */
    public FilteringBlockIterator<T> yLimit(int ly) {
        this.ly = ly;
        return this;
    }
     ...
     */
    public FilteringBlockIterator<T> team(Team team) {
        this.team = team;
        return this;
    }
     ...
     * optional param
     */
    public FilteringBlockIterator<T> max(int max) {
        this.max = max;
        return this;
    }
    ...
    public boolean hasNext() {
        ...
        boolean edgeOfGrid = x < 0 || x > grid.getWidthMax() || y < 0 || y > grid.getHeightMax();
        if (edgeOfGrid)
            return false;
    ...
    private void setNextBlock() {
        while (true) {
            ...
            if (dir2 != null) {
                boolean edgeOfGrid = cx < 0 || cx > grid.getWidthMax() || cy < 0 || cy > grid.getHeightMax();
    ...
    private boolean currentBlockPassesNonRegionalCriteria() {
        @SuppressWarnings("unchecked")
        T result = (T) grid.getBlockAt(x, y);
